...
import random
from baseline_conditions_RLAs import Random_base
    ...
    for sentence in sentences[0:]:
        ...
        if sent_id>1:
            ...
            for nodeinfo in sentence[0:]:                    # retrieves information of each node from dependency tree in UD format
                ...
                if not entry[3][1]=='PUNCT':
                    tree.add_node(entry[0][1], form=entry[1][1], lemma=entry[2][1], upostag=entry[3][1], xpostag=entry[4][1], head=int(list(entry[5][1].keys())[0]))                #adds node to the directed graph
            ROOT=0
            tree.add_node(ROOT)                            # adds an abstract root node to the directed graph

            #print(tree.nodes[1]['head'])
            for nodex in tree.nodes:
                print(tree.nodes[nodex])
                if not nodex==0:
                    if tree.has_node(tree.nodes[nodex]['head']):                                         # to handle disjoint trees
                        tree.add_edge(tree.nodes[nodex]['head'],nodex)       # adds edges as relation between nodes
